...
describe('`modulate` method', () => {
  test("`modulate` method must be defined, it takes 2 parameters (x and y) and returns the remainder of x divided by y as a number", () => {
    const solution = rewire("../solution");
    const Calculator = solution.__get__("Calculator");
    const calculator = new Calculator()
    expect(calculator.modulate(7, 5)).toEqual(2);
  });
});
